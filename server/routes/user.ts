...
import { SECRET, authenticateUserJwt } from '../middleware/auth';
...
router.post('/signup', async (req, res) => {
	...
	if (user) {
		...
		newUser.save();
		const token = jwt.sign({ email, role: 'user' }, SECRET as string, { expiresIn: '1h' });
...
router.get('/me', authenticateUserJwt, async (req, res) => {
	...
	const FirstName = await User.findOne({ email: email }).select('FirstName -_id')
	res.json(FirstName);
})
...
router.post('/verify', authenticateUserJwt, async (req, res) => {
	...
	if (isAuthentic) {
		...
		const payment = new Payment({ razorpay_order_id, razorpay_payment_id, razorpay_signature, email: email })
		payment.save()
